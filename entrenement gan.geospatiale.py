...
PATH = '/home/deo/Vidéos/datasetgeospatial'
...
#%%
checkpoint_dir = '/home/deo/Vidéos/datasetgeospatial\ckpt'
...
def generate_images(model, test_input, tar, htar, orig_inp, name):
    ...
    for i in range(out.shape.as_list()[0]):
        tf.io.write_file(os.path.join(r'/home/deo/Vidéos/datasetgeospatial',str(name)+'-'+str(i)+'.png'), tf.image.encode_png(out[i,:,:,:]))
...
def train_step(input_image, target, train_gen=True, train_dis=True):
    ...
    return disc_loss, gen_loss, vggl, tf.linalg.global_norm(discriminator_gradients), tf.linalg.global_norm(generator_gradients)
